...
const AgentField = ({
  ...
  React.useEffect(() => {
    users && setClients(users.filter((user) => user.role === "Agent"));
    users && setData(users.filter((user) => user.role === "Agent"));
    setValue(shipment && shipment?.agent);
  }, [users]);
  ...
  const onSelect = (index) => {
    if (index === 0) {
      ...
      handleChange("agent", data[index - 1].email);
    }
  };
